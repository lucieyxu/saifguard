...
def publish_dashboard_tool(report_text: str, gcp_project_id: str = "") -> str:
    """Publish a security report's findings to BigQuery for Looker Studio dashboarding.
    ...
    try:
        ...
        msg = f"Successfully published {len(table)} vulnerability record(s) to BigQuery dashboard table `{DASHBOARD_BQ_PROJECT}.{DASHBOARD_BQ_LOCATION}` for project `{target_project}`."
        LOGGER.info(msg)
        return msg
    except Exception as e:
        err_msg = f"Error when publishing to dashboard: {e}"
