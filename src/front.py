...
def respond_to_chat(input_str: str):
    """
    Call Agent with list of messages as multiturn conversations.
    """
    response = agent.invoke(message=input_str)
...
def on_load(e: me.LoadEvent):
    ...
    asyncio.run(agent.create_session(user_id=USER_ID, session_id=SESSION_ID))
...
def _submit_chat_msg():
    ...
    output_message = respond_to_chat(input)
